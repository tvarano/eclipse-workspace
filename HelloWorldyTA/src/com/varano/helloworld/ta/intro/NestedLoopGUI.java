...
public class NestedLoopGUI extends JPanel implements MouseListener, KeyListener
{
   ...
   public void paintComponent(Graphics g)
   {
      ...
      if(processDone && move < 8) g2.drawString("Complete.", getWidth()/2 - 100, 40);
      g2.setFont(new Font("Cooper Black", Font.PLAIN, 20));
      g2.setColor(Color.BLACK);      
      g2.drawString("clicks: " + click, getWidth()/2 - 40, 540);
      g2.setFont(new Font("Cooper Black", Font.PLAIN, 16));
      if(move > 0) g2.drawString("[–] to page back",  20, 580);
      if(move < 7) g2.drawString("[+] to page ahead", 440, 580);
...
/*16*/   for(int c=4; c>=4-r; c--) //MUST USE c HERE...just change values/conditions
   ...
   public void pyramid(Graphics2D g2)
   {
      ((JFrame)this.getTopLevelAncestor()).setTitle("pyramid");
      g2.drawString("pyramid", 50, 100);

/*15*/for(int r=4 ; r>=0; r--)   //MUST USE r HERE...just change values/conditions
      {
/*16*/   for(int c=2-r; c<=2+r; c++) //MUST USE c HERE...just change values/conditions
         {
            if(loop < click)
               drawBox(g2, r, c);
            loop++;
   ...
   private static void createAndShowGUI() {
      NestedLoopGUI gamePanel = new NestedLoopGUI();

      JFrame frame = new JFrame("Click and Draw!");
